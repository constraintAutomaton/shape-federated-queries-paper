...
One querying paradigm over this distributed graph is #abbr.a("LTQP") @hartig2012foundations, in which an engine dynamically discovers relevant data by following RDF links at runtime.
...
SPLENDID @gorlitz2011splendid, a pioneer this direction, by exploits VoID dataset descriptions @w3DescribingLinked to identify relevant endpoints at planning time.
The second issues analytic queries directly against endpoints.
FedX @schwarte2011fedx, started this research line using an approach sending ASK queries at runtime to determine which endpoints can contribute to each triple pattern.
